...
$allFaculty = $departmentModel->getAllActiveFaculty();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($data['name'])) {
        ...
        $existing = $departmentModel->getByCode($data['code']);
        if ($existing && $existing['id'] != $dept_id) {
            $error = 'Department code already exists';
        } else {
            if ($departmentModel->update($dept_id, $data)) {
                $auth->logActivity($auth->getAdminId(), 'UPDATE_DEPARTMENT', 'Updated department: ' . $data['name']);
...
<head>
    ...
    <title>Edit Department - GyanPeeth College</title>
...
<body>
    ...
    <div class="admin-content">
        ...
        <div class="content-area">
            ...
            <div class="current-hod-info">
                <i class="fas fa-user-tie"></i> 
                <strong>Current HOD:</strong> 
                <?php echo htmlspecialchars($dept['hod_name']); ?>
                (<?php echo htmlspecialchars($dept['hod_designation']); ?>)
            </div>
            <?php endif; ?>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="faculty-select-info">
                                        ...
                                        <strong>Note:</strong> You can change the HOD or select "No HOD Selected" to remove the current HOD.
                                        Faculty must be added first before they can be assigned as HOD.
                                    </div>
                                </div>
                            </div>
